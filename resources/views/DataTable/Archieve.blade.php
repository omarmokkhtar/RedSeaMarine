                ...
                <div class="box-body">
                  ...
                  @foreach($clients as $client)
                      <tr>
                        ...
                        <td>{{$client->container_number}}</td>
                        <td>{{$client->box_number}}</td>
                        <td>{{$client->policy_no}}</td>
                        <td>{{$client->type}}</td>
                        <td>{{$client->powerOfAttorney}}</td>
                        <td>{{$client->doc_date}}</td>
                        <td>
                          <ul style="list-style-type: none;">
                            <li><a href="{{URL::to('currArchive/'.$client->id)}}"><button class="btn btn-block btn-primary btn-sm">عرض المزيد</button></a></li>
                            <li><a id="finReport" href="{{URL::to('finArchive/'.$client->id)}}"><button class="btn btn-block btn-success btn-sm"> الجدول المالي</button></a></li>
                            <li><a href="{{URL::to('episodeBack/'.$client->id)}}"><button class="btn btn-block btn-danger btn-sm">أستعادة</button></a></li>
                          </ul>
                        </td>
                        <!--<td><a href="{{URL::to('currArchive/'.$client->id)}}"><button class="btn btn-block btn-primary btn-sm">عرض المزيد</button></a></td>
                        <td><a id="finReport" href="{{URL::to('finArchive/'.$client->id)}}"><button class="btn btn-block btn-success btn-sm"> الجدول المالي</button></a></td>
                        <td><a href="{{URL::to('episodeBack/'.$client->id)}}"><button class="btn btn-block btn-danger btn-sm">أستعادة</button></a></td>-->
                      </tr>
                      @endforeach
